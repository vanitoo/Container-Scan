...
class MainWindow:
    ...
    def apply_match_results(self, match_results):
        ...
        for result in match_results:
            ...
            while len(values) < 7:
                values.append("")
            values[6] = self.components["table"].format_differences(values[3], values[4])
    ...
    def _show_save_summary(self, output_dir: Path, message: str, has_errors: bool):
        ...
        dialog.grab_set()
        ttk.Label(dialog, text=message, justify=tk.LEFT, padding=(16, 14)).pack(fill=tk.BOTH, expand=True)
        ...
        ttk.Button(buttons, text="Открыть папку", command=open_results_folder).pack(side=tk.RIGHT, padx=(0, 6))
    ...
    def _save_results_worker(self, btn):
        try:
            ...
            def _close_progress_ok(output_dir: Path, saved_count: int, unrecognized_pages: list[int], failed_pages: list[int], total: int):
                try:
                    if ui.get("win"):
                        ui["win"].destroy()
                finally:
                    error_count = len(unrecognized_pages) + len(failed_pages)
                    message = (
                    ...
                    if unrecognized_pages:
                        message += "\n\nНе распознаны страницы: " + ", ".join(map(str, unrecognized_pages))
                    if failed_pages:
                        message += "\nНе удалось сохранить страницы: " + ", ".join(map(str, failed_pages))
                    self._show_save_summary(output_dir, message, bool(error_count))
            ...
            unrecognized_pages = []
            failed_pages = []
            invalid_names = {"", "не распознано", "не распознано.", "не распознан", "не распознана"}
            ...
            for i, entry in enumerate(self.state.table_entries):
                try:
                    ...
                    valid_match = match_cell if match_cell.casefold() not in invalid_names else ""
                    valid_recognized = recognized if recognized.casefold() not in invalid_names else ""
                    basename = valid_match or valid_recognized

                    if not basename:
                        unrecognized_pages.append(i + 1)
                        logger.warning(f"Страница {i + 1} не сохранена: контейнер не распознан")
                    else:
            ...
            self.root.after(
                0,
                lambda d=output_dir, saved=saved_count, unrec=unrecognized_pages, failed=failed_pages, count=total:
                    _close_progress_ok(d, saved, unrec, failed, count),
            )
    ...
    def check_image(self):
        ...
        if (not self.state.selected_areas and
                all(v is not None for v in [self.state.x_start, self.state.y_start, self.state.x_end, self.state.y_end])):
            self.state.selected_areas = [
    ...
    def _update_table_row(self, page_num, recognized_text):
        ...
        if page_num < len(self.state.recognition_results):
            self.components["canvas"].show_recognition_result(self.state.recognition_results[page_num])
